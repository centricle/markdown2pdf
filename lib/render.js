...
/*
 * Raw HTML is allowed through, and scripts are disabled at print time
 * (lib/print.js), but some elements need no script to misbehave. An
 * `<iframe src="file:///...">` pastes a local file into the output, `<object>`
 * and `<embed>` do the same, and a `<meta http-equiv="refresh">` can navigate
 * the page somewhere else before it prints. Nothing a PDF needs comes through
 * any of them, so they are dropped. Code blocks are unaffected because their
 * angle brackets are entity-escaped before this runs.
 */
const UNPRINTABLE_ELEMENTS = /<\/?(?:iframe|frame|frameset|object|embed|portal|meta)\b[^>]*>/gi;

/** Markdown source to an HTML fragment. */
export function renderBody(markdown) {
  return createRenderer().render(markdown).replace(UNPRINTABLE_ELEMENTS, '');
}
