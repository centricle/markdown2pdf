...
/**
 * The stylesheet is inlined into a `<style>` element verbatim, so a closing
 * tag inside it would end the element early and turn the rest of the file
 * into markup. There is no legitimate reason for that sequence in CSS.
 */
export function readTheme(theme) {
  const css = readFileSync(theme.path, 'utf8');
  if (/<\/style/i.test(css)) {
    throw new ThemeError(`Theme ${theme.name} contains "</style", which is not valid in a stylesheet.`);
  }
  return css;
}
